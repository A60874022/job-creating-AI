...
def dialogue_list(request):
    ...
    try:
        ...
        logger.error("Error loading dialogue list for user %s: %s", request.user.id, str(e), exc_info=True)
...
def dialogue_detail(request, dialogue_id):
    ...
    try:
        ...
        logger.error("Error in dialogue detail for dialogue %s, user %s: %s", 
                    dialogue_id, request.user.id, str(e), exc_info=True)
...
def send_message(request, dialogue_id):
    ...
    try:
        ...
        if request.user not in [dialogue.user1, dialogue.user2]:
            logger.error("User %s attempted to send message to unauthorized dialogue %s", 
                        request.user.id, dialogue_id)
        ...
        logger.error("Error sending message in dialogue %s by user %s: %s", 
                    dialogue_id, request.user.id, str(e), exc_info=True)
...
def mark_messages_read(request, dialogue_id):
    ...
    try:
        ...
        # Проверка прав доступа
        if request.user not in [dialogue.user1, dialogue.user2]:
            logger.error("User %s attempted to mark messages read in unauthorized dialogue %s", 
                        request.user.id, dialogue_id)
            return JsonResponse({'status': 'error', 'message': 'No permission'})
        ...
        logger.error("Error marking messages read in dialogue %s by user %s: %s", 
                    dialogue_id, request.user.id, str(e), exc_info=True)
        return JsonResponse({'status': 'error', 'message': 'Internal server error'})
...
def delete_dialogue(request, dialogue_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            NotificationService.delete_dialogue_notifications(dialogue.user1, dialogue_id)
            NotificationService.delete_dialogue_notifications(dialogue.user2, dialogue_id)
        ...
        logger.error("Error deleting dialogue %s by user %s: %s", 
                    dialogue_id, request.user.id, str(e), exc_info=True)
...
def clear_all_dialogues(request):
    ...
    try:
        if request.method == 'POST':
            ...
            for dialogue in dialogues:
                NotificationService.delete_dialogue_notifications(dialogue.user1, dialogue.id)
                NotificationService.delete_dialogue_notifications(dialogue.user2, dialogue.id)
                dialogue.messages.all().delete()
                dialogue.delete()
        ...
        logger.error("Error clearing all dialogues for user %s: %s", 
...
def start_dialogue_from_product(request, product_id):
    ...
    try:
        ...
        logger.error("Error starting dialogue from product %s by user %s: %s", 
