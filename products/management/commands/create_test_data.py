...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        products_data = [
            (users['master_anna@example.com'], categories['Вышивка'], 
             'Вышитая картина "Лесная фея"', 
             'Ручная вышивка мулине на хлопковом полотне. Размер 30x40 см.', 4500),
            (users['master_anna@example.com'], categories['Вышивка'],
             'Вышитые серьги "Цветочные"',
             'Нежные серьги с вышитыми цветами. Легкие и удобные.', 1200),
            (users['master_peter@example.com'], categories['Керамика и гончарное дело'],
             'Керамическая кружка "Утро"',
             'Ручная лепка, уникальная глазурь. Объем 350 мл.', 1800),
            (users['master_peter@example.com'], categories['Керамика и гончарное дело'],
             'Набор тарелок "Геометрия"',
             'Набор из 4-х тарелок разного диаметра. Ручная роспись.', 3200),
            (users['master_anna@example.com'], categories['Вязание и кружево'],
             'Вязаный свитер "Северный"',
             'Теплый свитер из натуральной шерсти. Размер M.', 5200),
            (users['master_peter@example.com'], categories['Украшения'],
             'Керамическое колье "Луна"',
             'Подвеска из керамики ручной работы на кожаном шнурке.', 1500),
            (users['master_anna@example.com'], categories['Декор для дома'],
             'Вышитая подушка "Винтаж"',
             'Декоративная подушка с ручной вышивкой.', 2800),
        ]
        ...
        Order.objects.filter(customer__in=[users['customer_maria@example.com'], 
                                         users['customer_alex@example.com']]).delete()
        
        order1, created = Order.objects.get_or_create(
            customer=users['customer_maria@example.com'],
            status='оформлен'
        )
        if created:
            OrderItem.objects.get_or_create(
                ...
                defaults={
                    'quantity': 1,
                    'price_at_moment': products[0].price
                }
            )
        ...
        order2, created = Order.objects.get_or_create(
            customer=users['customer_alex@example.com'], 
            status='в_работе'
        )
        if created:
            OrderItem.objects.get_or_create(
                ...
                defaults={
                    'quantity': 1,
                    'price_at_moment': products[2].price
                }
            )
            OrderItem.objects.get_or_create(
                order=order2,
                ...
                defaults={
                    'quantity': 1,
                    'price_at_moment': products[5].price
                }
            )
        ...
        Favorite.objects.filter(user__in=[users['customer_maria@example.com'], 
                                        users['customer_alex@example.com']]).delete()
        
        Favorite.objects.get_or_create(
            user=users['customer_maria@example.com'], 
            product=products[1]
        )
        Favorite.objects.get_or_create(
            user=users['customer_maria@example.com'], 
        ...
        Favorite.objects.get_or_create(
            user=users['customer_alex@example.com'], 
            product=products[3]
        )
